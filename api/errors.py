...
def handle_model_not_found(error_message, window, settings, fallback_callback):
    """Show model-not-found error with a 'Select Model' button in chat.
    ...
    Args:
        error_message: Raw API error (starts with "model:").
        window: The Sublime Text window (may be None).
        settings: The plugin settings object.
        fallback_callback: Called with a plain error if window is unavailable.
    """
    ...
    if window:
        ...
        message_end_position = claudette_chat_status_message(
            window, display_message, "⚠️"
        )
        if message_end_position >= 0:
            try:
                chat_view_instance = ClaudetteChatView.get_instance(
                    window, settings
                )
                if chat_view_instance:
                    chat_view_instance.add_select_model_button(
        ...
        fallback_callback(
            ...
            "Settings > Package Settings > Claudette > Select Model.".format(
                display_message
            )
        )
