...
def parse_web_search_items(items):
    ...
    Returns:
        tuple: (list of markdown link strings, bool indicating if there was an error)
    """
    lines = []
    has_error = False
...
def format_search_results(source_lines):
    ...
    Args:
        source_lines: List of markdown link strings (e.g. ["- [Title](url)", ...]).

    Returns:
        str: Formatted markdown section with heading, or empty string if no lines.
    """
    if not source_lines:
        return ""
